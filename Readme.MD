...
- `APP_ADMIN_TOKEN` / `ADMIN_TOKEN` (Bearer token granting `admin` role for moderation)
- `APP_MODERATOR_TOKEN` / `MODERATOR_TOKEN` (Bearer token granting `moderator` role for moderation endpoints)
