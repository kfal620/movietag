...
@router.get("/scene-attributes")
def list_scene_attributes(
    verified_only: bool = Query(default=False, description="Only show user-verified attributes"),
    attribute: str | None = Query(default=None, description="Filter by attribute name"),
    limit: int = Query(default=100, ge=1, le=500),
    offset: int = Query(default=0, ge=0),
    db: Session = Depends(get_db),
) -> dict[str, Any]:
    """List all scene attributes across all frames.
    
    Shows attribute name, value, confidence, user-verified status, and frame context.
    Useful for exploring which attributes were edited by users vs. generated by AI.
    """
    query = (
        db.query(SceneAttribute)
        .options(joinedload(SceneAttribute.frame))
    )
    
    if verified_only:
        query = query.filter(SceneAttribute.is_verified == True)
    if attribute:
        query = query.filter(SceneAttribute.attribute == attribute)
    ...
    attributes = (
        query
        .order_by(SceneAttribute.is_verified.desc(), SceneAttribute.updated_at.desc())
        .offset(offset)
        .limit(limit)
        .all()
    )
    ...
    for attr in attributes:
        items.append({
            ...
            "frame": {
                "id": attr.frame.id,
                "movie_title": attr.frame.movie.title if attr.frame.movie else None,
                "status": attr.frame.status,
            } if attr.frame else None,
        })
...
@router.delete("/scene-attributes/{attribute_id}")
def delete_scene_attribute(
    attribute_id: int,
    db: Session = Depends(get_db),
    _: object = Depends(require_role("moderator", "admin")),
) -> dict[str, Any]:
    """Delete a specific scene attribute.
    
    Allows removal of individual attribute values, whether user-verified or AI-generated.
    """
    attribute = db.get(SceneAttribute, attribute_id)
    if not attribute:
        raise HTTPException(status_code=404, detail="Scene attribute not found")
    
    frame_id = attribute.frame_id
    attr_name = attribute.attribute
    attr_value = attribute.value
    was_verified = attribute.is_verified
